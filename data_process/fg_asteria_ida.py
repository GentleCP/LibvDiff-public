...
# 日志配置
logger = LogHandler(name="ASTGenerator", log_path=ROOT_PATH.joinpath('log/'), file=True, stream=False,
                    level=cptools.INFO)
...
FUNC_BLACK_LIST = {'dsa_builtin_paramgen2', 'dsa_builtin_paramgen', 'blake2b_compress', 'FT_Stroker_CubicTo'}
...
class ASTGenerator(object):
    ...
    def generate(self):
        ...
        logger.info(
            "Analysis result -> {}/{}/{}(success/skip/total func num)".format(self.success_func_num, self.skip_func_num,
                                                                              self.total_func_num))
        print(f'finished, time cost:{time.time() - start}')
