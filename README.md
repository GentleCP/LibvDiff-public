...
- basic features: function names (including exports), string literals. They are organized as `.json` files.
```shell
# func_names.json
{
  "exports": [f1, f2, ...],
...
{
  "strings_all": ["s1", "s2", ...]
  "strings_in_rodata": ["s1,", "s2", ...]  
}
```
- function embeddings: They are vectors generated by Asteria, store as a python pickle file and can be loaded as dict
```shell
# Asteria_embeddings.pkl
{
  f1: [0.123, 0.124, ...],
  ...
}
```
- anchor paths: They are store in a `.csv` file with 5 fields `export`(The exported function), cur_node (the start function), path (anchor path), path_seq (CDS), path_len (the length of path). For faster retrieval, we transform the anchor path into two mapping dict as follows:
- `anchor_path-ep2nodes.json`: The key is the export with CDS, and the value is the nodes that can be reached.
```shell
{
  export1@['e', 'e', 'r']: [
    f1, f2, ...
...
```
- `anchor_path-node2eps.json`: The key is the nodes, and the value is the export with CDS
```shell
{
  f1: [
    export1@['e', 'e', 'r'],
...
- `vdcs_generator.py` generate the version differences of input OSS. The version differences are represented as two `.json` files.
```shell
# vp_diffs-func-lib.json
{
  "v1@v2": [
    [add_func1, add_func2, ...],
    [delete_func1, delte_func2, ...],
    [update_func1, update_func2, ...]
  ]
}

# vp_diffs-str-lib.json
{
  "v1@v2": [
    [add_str1, add_str2, ...],
    [delete_str1, delte_str2, ...],
  ]
}
```
...
```
- `vct_generator.py` generates the version coordinates of each version of input OSS. The version coordinates can be represented as a `.csv` file with 4 fields, `version` , `vc_x` (X-axis), `vc_y` (Y-axis), `ver_index` (index of version)
